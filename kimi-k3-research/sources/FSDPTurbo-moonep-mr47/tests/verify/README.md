...
CUDA 必须把静态 buffer 尾部折进最后一组（`offs[-1] == NvS`）。
`65d9542` 按 `cu_seqlens` 截断、不再 mask，H20 上 `torch._grouped_mm`
会 `unspecified launch failure`，其它 rank 卡在 MoonEP
`cross_rank_barrier` 100s。GPU 只做功能验证，不再为峰值去改这条路径。
NPU 显存收益看 OPT-1（`[E+B]` 映射）和 OPT-2（GMM 反向写入 `main_grad`），
不看 CUDA 这条 tail 优化。NPU `npu_grouped_matmul` 同样要求 group list
覆盖整段 buffer，继续 mask + fold。
