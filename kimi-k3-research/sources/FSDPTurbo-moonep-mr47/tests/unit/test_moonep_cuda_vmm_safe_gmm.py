"""MoonEP CUDA grouped matmul must not call F.grouped_mm."""
...
def test_sink_forces_vmm_safe_path_without_the_flag(monkeypatch):
    torch.manual_seed(0)
    inputs = torch.randn(9, IN_FEATURES, requires_grad=True)
    ...
    def forbidden(*args, **kwargs):
        calls.append(1)
        raise AssertionError("F.grouped_mm must not run when a MoonEP sink is set")
